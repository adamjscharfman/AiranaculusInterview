...
def apply_frequency_shift(iq_data:np.array,fc_offset:float,fs:float)->np.array:
...
def compute_average_phase(iq_data:np.array,samples_per_symbol:int,offset:int=0)->float:
...
def estimate_m_psk_rotation(iq_data:np.array,num_bits_per_symbol:int)->float:
    m_psk = 2**num_bits_per_symbol
    rotation = 1/m_psk * np.angle(np.sum(iq_data**m_psk))
    return rotation

def estimate_bpsk_frequency_offset_one_lag_estimator(iq_data:np.array,samples_per_symbol:int)->float:
